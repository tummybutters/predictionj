...
export default async function JournalDataPage({
  searchParams,
}: {
  ...
  let dbReady = true;
  try {
    items = await listByType(ensured.user_id, "data", { limit: 120 });
  } catch {
    dbReady = false;
  }
  ...
  async function createData() {
    ...
    redirect(`/journal/data?id=${encodeURIComponent(created.id)}`);
  }
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 px-6 pb-16 pt-8">
      <PageHeader
        title="Data"
        subtitle="Anything you want to keep as evidence, quotes, or source material."
        actions={
      ...
      <div className="grid gap-4 lg:grid-cols-[340px_1fr]">
        <Panel className="p-3">
          <div className="flex items-center justify-between px-2 pb-2 pt-1">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted">Data</div>
          ...
          <div className="space-y-2">
            {items.map((b) => {
              ...
              return (
                <Link
                  key={b.id}
                  href={`/journal/data?id=${encodeURIComponent(b.id)}`}
                  className={cn(
            ...
            {items.length === 0 ? (
              <div className="rounded-2xl border border-border/15 bg-panel/35 p-4 text-sm text-muted">
                Paste an article, a snippet, or any evidence you want to reference later.
              </div>
            ) : null}
          </div>
        </Panel>
